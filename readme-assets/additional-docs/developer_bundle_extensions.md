...
The `launch` tool will prompt for a selection of a `.kit` file to launch. Select the desired UI based application. The developer bundle is not currently suitable for headless service applications.
...
**:warning: The developer extensions do require a UI based application to run properly. They will not work as expected for headless service applications**